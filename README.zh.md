...
| 特性 | OrionDesk | Electron | Tauri | Qt |
|------|-----------|----------|-------|-----|
| **安装包大小** | ~3MB | ~150MB | ~10MB | ~50MB |
| **内存占用** | ~30MB | ~200MB | ~50MB | ~100MB |
| **启动时间** | <1秒 | 2-5秒 | 1-3秒 | 3-10秒 |
| **原生 API 访问** | ✅ 完整 | ✅ 完整 | ✅ 完整 | ✅ 完整 |
| **Web 技术栈** | ✅ Vue/React | ✅ Vue/React | ✅ Vue/React | ❌ |
| **内置 SQLite** | ✅ 开箱即用 | ⚡ 需插件 | ⚡ 需插件 | ⚡ 需插件 |
| **国际化支持** | ✅ 内置 | ⚡ 需插件 | ⚡ 需插件 | ⚡ 需插件 |
| **学习曲线** | 低 | 中 | 中 | 高 |

### 🎯 我们的优势

**💡 轻量但强大**
OrionDesk 在保持安装包小于 5MB 的同时，提供原生桌面体验。无 Chromium 开销，无需重型运行时。

**🔰 入门友好**
如果你会 Vue.js，就能开发桌面应用。无需学习 Rust (Tauri) 或 C++ (Qt)，熟悉的 Vue 3 + TypeScript 工作流。

**🛠️ 生产就绪**
SQLite 数据库、多语言支持、自定义标题栏、窗口管理 - 全部内置，开箱即用。

**⚡ 闪电般的速度**
冷启动不到 1 秒。开发时热重载。你的用户无需等待。

## 🎯 适用场景

| 场景 | 为什么选择 OrionDesk |
|------|---------------------|
| **开发者工具** | 轻量级，快速启动 - 工具即时启动 |
| **实用程序** | 小安装包，低内存 - 任意环境运行 |
| **数据管理应用** | 内置 SQLite - 无需外部数据库 |
| **内部工具** | 跨平台 - 所有团队机器都能运行 |
| **MRP/ERP 系统** | Vue 3 + TypeScript - 可扩展架构 |
| **IoT 仪表盘** | 实时能力 - 内置 WebSocket 支持 |
